		...
		function quickSandPaginated( settings ){
			...
			function updatePortfolio(filter_category, page, no_quicksand) {       //TODO: this function need to be break in pieces
				...
				$(settings.pageNumberContainer+' a').click(function(event) {
					...
					var current_filter = $('.actv').attr('data-category');
			...
			updatePortfolio($(settings.filtersContainer).find('a.actv').attr('data-category') || false, 1, false);
			portfolio_loaded = true;
		
			// Category filter click
			$(settings.filtersContainer).find('a').hover(function(){
				$(settings.filtersContainer).find('a#actv').removeClass('actv');
			}, function(){
				$(settings.filtersContainer).find('a#actv').addClass('actv');
			}).click(function(event) {
				event.preventDefault(); // stop regular click action
				// Clicks work only after page load effects are complete
					// Highlight clicked link
					$(settings.filtersContainer).find('a').removeAttr('id').removeClass('actv');
					$(this).attr({'id':'selected'}).addClass('actv');
